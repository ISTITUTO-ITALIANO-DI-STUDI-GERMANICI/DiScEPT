...
export default function AlignTab({id, onSelectionChanged}) {
  ...
  const handleChange = (event) => {
    ...
    contentRef.current.append(CETEIHelper.CETEI.makeHTML5(
      data.getDocumentPerLanguage(event.target.value), null,
  ...
  const alignLogic =(id, rootElm, domElm, teiElm) => {
    domElm.addEventListener("click", e => {
      e.target.classList.toggle("selectedTEI");

      onSelectionChanged(Array.from(rootElm.getElementsByClassName("selectedTEI")));
      ...
      <FormControl fullWidth>
        ...
        >{
          data.getDocumentLanguages().map(language => (
            <MenuItem value={language} key={id + "-key-" + language}>{language}</MenuItem>
          ))}
        </Select>
      </FormControl>
